...
const N={'감정격화':'감정격화·위협표현','책임·형평성':'책임·형평성불만','처리지연':'처리지연·장기방치'};
const rows=[];for(const m of md.matchAll(/^\d+\. (.+?) \| (.+?) \| `(.+?)` \| \[(.*?)\]/gm))rows.push({trade:m[1].trim(),dtype:m[2].trim(),content:m[3],gold:m[4].split(',').map(x=>x.trim()).filter(Boolean).filter(x=>!x.startsWith('없음')).map(x=>N[x]||x)});
...
rows.forEach((r,i)=>{const g=new Set(r.gold),o=new Set(res[i].got.filter(x=>x!=='사설시공·인테리어'));   /* 760차: 요소 10종으로 합침 — 정답셋 이름도 갱신. 사설시공은 정답셋 이후 추가라 비교에서 뺀다 */const ok=g.size===o.size&&[...g].every(x=>o.has(x));if(ok)exact++;else miss.push(r.content.slice(0,50)+' | 정답 '+r.gold.join(',')+' | 판정 '+res[i].got.join(','));for(const x of o)g.has(x)?tp++:fp++;for(const x of g)if(!o.has(x))fn++;});
...
const ctx=await pg.evaluate(()=>[
  ...
  ['지난번에도 문의했는데 아직 답변이 없습니다',['반복하자·반복민원','응대·소통불만']],['방문 이후 처리 예정',[]],['보수했으나 동일 현상 발생',['반복하자·반복민원']],
  ['현대에서 직접 와서 봐라',['응대·소통불만']],['담당자 연락해라',['응대·소통불만']],['협력업체 본사에 연락함',[]],['인테리어 줄눈 시공분 보상 문의',['사설시공·인테리어']],['사설 중문 설치 후 문틀 파손',['사설시공·인테리어']],['줄눈 파편이 떨어짐',['안전위험']],['거실 줄눈 균열',[]],
  ['몇 달째 처리되지 않고 있습니다',['처리지연·장기방치']],['작년부터 신청했는데 아직도 조치가 안됨',['처리지연·장기방치']],['아직 방문하지 않음',['처리지연·장기방치']],['8/21 // 8/30 / 9/2 독촉',['반복하자·반복민원']],['고객 보상 요구함, 자재발주 처리예정',['보상요구']],['자재발주 처리예정',[]],['자재발주했으나 고객이 보상을 요구함',['보상요구']],['8/21 접수 / 8/30 현장 방문',[]],

...
| `calapp/tasks/{subjectId}/{itemId}` | 주요업무 항목 — text(제목)·body(내용)·st(0예정 1진행 2완료 3보류)·due·order·assignees·links·comments |
| `calapp/prefs/{uid}` | 개인 설정 — 저장한 필터(본인만 쓰기) |
...
**주요업무 현황** — 왼쪽에서 대상(팀 공통업무 · 담당자)을 고르고 오른쪽에서 업무를 작성·확인한다.
담당자는 권역 칩으로 좁혀 볼 수 있다. 업무에는 제목·내용과 함께 담당자를 여러 명 지정하고
참고 링크를 붙일 수 있으며, 끌어서 순서를 바꾼다. 코멘트에 `@이름`을 쓰면 그 사람에게 알림이 간다.

...
| 우클릭한 곳 | 할 수 있는 일 |
|---|---|
| 달력 날짜 | 이 날짜에 업무 추가 · 이 날짜로 이동 · **단체연차 지정/해제**(관리자) |
| 업무 카드 | 완료 ↔ 진행 · 수정 · 제목 복사 · 삭제 |

**단체연차 같은 팀 휴무일**은 지정하면 **공휴일과 같은 색으로 칠해지고** 날짜 위에 이름이 붙습니다
(글자 색은 공휴일과 달라 구분됩니다). 지정하는 방법이 둘입니다.

- **한 날짜만** — 달력에서 그 날짜를 우클릭 > 단체연차로 지정
- **여러 날 한 번에** — 설정 > **휴무일** 에서 시작일·종료일을 넣고 추가.
  연초에 한 해치를 몰아서 넣을 때 씁니다. **주말과 공휴일은 알아서 건너뜁니다.**

설정 > 휴무일에는 지정된 날이 연도별로 나오고, 지난 날짜는 흐리게 표시됩니다.

...
**주요 업무** — 주 단위 업무보고용 표.
**완료**(그 주에 끝난 업무)와 **진행**(아직 안 끝난 업무)을 나눠, 권역 → 담당자 → 업무 순으로 늘어놓는다.
열은 권역 · 담당자 · 현장 · 업무 · 진행상태 · 완료일 · 비고이고, 업무가 없는 권역·담당자도 빈 줄로 남긴다.
권역 이름에 '미인수'가 들어가면 보고 대상에서 뺀다.
완료일은 업무의 종료일(없으면 날짜)이고, 비고는 처리계획(없으면 진행경과)을 가져온다.
왼쪽 위 화살표로 주를 옮기고, [인쇄 · PDF] 로 표만 뽑을 수 있다.

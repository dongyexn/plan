...
담당자 지정과 자동 색, 권역 세그먼트 + 담당자 선택 필터(권역을 고르면 담당자 목록도 좁혀진다). 기한이 있는 미완료 주요업무는 점선 배지로 함께 보인다.
...
**조직 관리(관리자)** — 팀 · 권역 · 현장을 각각 등록하고, 계정에 팀 · 권역 · 담당 현장 · 권한을 지정한다.
하자처리 대시보드의 최신 게시본(`report/{기준월}/_dash`)에 담긴 팀 · 권역 · 현장 목록을
**불러오기** 한 번으로 가져올 수 있다. 이름이 같은 항목은 기존 계정 배정이 그대로 이어진다.

...
**내 업무** — 첫 열에 작은 달력과 받은 멘션, 나머지 열에 팀 공통 업무와 내 주요업무.
작은 달력은 내 업무나 팀 공통 업무가 걸친 날 아래에 점을 찍고, 날짜를 누르면 캘린더의 그 날로 간다.
목록의 업무를 누르면 그 업무가 있는 화면으로 이동한다.
...
팀 전체를 고르면 목록이 두 열로 나뉜다 — 왼쪽은 공통 업무와 팀장, 오른쪽은 권역 담당자.
달력에서 팀 공통 업무(담당자가 없는 업무)는 막대 앞에 별(★)이 붙는다.

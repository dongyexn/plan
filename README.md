...
- **보류한 업무**는 넘긴 업무가 모이는 곳입니다(회색 원 안 − 아이콘). 캘린더 화면 오른쪽 아래에 있고(**내 업무만 보기**로 좁힐 수 있습니다),
  **업무를 달력 날짜로 끌어다 놓으면 그 날짜로 되살아납니다.** 업무 목록 화면에서는 왼쪽 **보류한 업무**를 눌러 전체를 모아 볼 수 있습니다.
...
- 화면 오른쪽 위에 **기준월**과 **인쇄** 버튼이 나옵니다. 모든 숫자는 기준월 말일 기준입니다.
- **처리계획**은 이 앱에서도 작성할 수 있고, 저장하면 팀 전체에 바로 반영됩니다.
- 조직/현장 관리의 **공가세대·공가상가** 표시는 하자처리현황에서 관리하는 값이라 이 앱에서는 읽기 전용입니다.
